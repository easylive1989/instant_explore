...
void main() {
  ...
  group('getJourneyEntries', () {
    ...
    test('遠端網路錯誤且有快取時，回傳快取資料', () async {
      // Arrange
      final cached = [_createTestEntry('cached-1', userId)];
      when(() => mockRemote.getJourneyEntries(userId)).thenThrow(
        const AppError(
          type: JourneyError.networkError,
          message: 'Network error',
        ),
      );
      when(() => mockCache.getEntries(userId)).thenAnswer((_) async => cached);
    ...
    test('遠端網路錯誤且無快取時，重新拋出錯誤', () async {
      // Arrange
      const networkError = AppError(
        type: JourneyError.networkError,
        message: 'Network error',
      );
      when(() => mockRemote.getJourneyEntries(userId)).thenThrow(networkError);
      when(() => mockCache.getEntries(userId)).thenAnswer((_) async => null);

      // Act & Assert
      expect(() => sut.getJourneyEntries(userId), throwsA(isA<AppError>()));
    });

    test('遠端非網路錯誤時，重新拋出錯誤且不嘗試讀取快取', () async {
      // Arrange
      when(() => mockRemote.getJourneyEntries(userId)).thenThrow(
        const AppError(type: JourneyError.loadFailed, message: 'Load failed'),
      );
    ...
    test('遠端成功但快取存儲失敗時，仍回傳遠端資料', () async {
      // Arrange
  ...
  group('addJourneyEntry', () {
    ...
    test('遠端失敗時拋出例外，且不呼叫 cache.addEntry', () async {
      ...
      when(() => mockRemote.addJourneyEntry(entry)).thenThrow(
        const AppError(type: JourneyError.saveFailed, message: 'Save failed'),
      );
  ...
  group('deleteJourneyEntry', () {
    test('遠端成功時，呼叫 remote.deleteJourneyEntry', () async {
      // Arrange
