...
> `lorescape/`**，不是這個 skill 資料夾。metrics 程式在
> `lorescape/scripts/metrics/`。指令請在專案根目錄下執行，例如
> `cd /Users/paulwu/Documents/PLRepo/lorescape/scripts`。
...
2. **先 dry-run** 檢查設定與待補進度（讀 CSV 算缺口，不抓 API）：
       ...
       cd <lorescape 專案根>/scripts && uv run python -m metrics.report --check
...
3. 抓 API 來源並寫進 CSV：
       ...
       cd <lorescape 專案根>/scripts && uv run python -m metrics.report
