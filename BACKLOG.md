...
- ⚠️ 這是 Flutter 改動，需隨下次 App build 送審才生效（與 F13 T1c 同一版）
- [x] T1a: 匯入 v2 handoff bundle 到 `docs/design/`（2026-07-21）
- [ ] T1: 導入手寫體字型 Long Cang（拍立得圖說用）到 pubspec assets
- [ ] T2: `JourneyScreen` 換成 Masthead（眼眉線「旅程紀錄 · Chronicle」+
  標題「歷程」+ 新增鈕）＋ 旅程書架（`.shelf` 立體書背、層板、四色書皮、
  高度依序變化）；未分類也是一本書。移除分段控制與 `_JourneyList`
- [ ] T3: 手記翻頁單頁 `NotebookPage`——紙張質感（裝訂孔、頁邊線）、
  拍立得照片＋膠帶（奇偶頁左右交替傾斜）、Long Cang 手寫圖說、
  日期戳、頁碼與分享/刪除動作
- [ ] T4: 翻頁器 `NotebookPager`——水平拖曳翻頁（門檻 60px、兩端橡皮筋
  0.32 阻尼、`cubic-bezier(.22,.61,.36,1)` 0.4s）與頁點指示器
- [ ] T5: `TripDetailScreen` 換成翻頁器，**且不得弄丟既有功能**：多選模式、
  移動到其他旅程、PDF 匯出、刪除
- [ ] T6: 移除時間軸後的殘留盤點——`journeyViewModeProvider`、
  `journeySearchQueryProvider`、`TimelineEntry` 是否還有其他使用者；
  更新受影響測試；`fvm flutter analyze --fatal-infos`
