...
| 欄位 | 填法 | 說明 |
|------|------|------|
| **Identifier** | `com.paulchwu.instantexplore.premium_monthly` | 與 App Store Connect 的 Product ID 完全一致（iOS 無 40 字元限制） |
...
| **Subscription**（Product ID）| `instantexplore.premium_monthly` | 與 Google Play Console 的訂閱 Product ID 一致（最多 40 字元） |
...
2. 填入：
   ...
   - **產品 ID**：`com.paulchwu.instantexplore.premium_monthly`（建立後無法修改，iOS 無字元數限制）
...
3. 點選 **建立訂閱**
4. 填入：
   - **產品 ID**：`instantexplore.premium_monthly`（最多 40 字元，建立後無法修改）
   - **名稱**：Premium Monthly
