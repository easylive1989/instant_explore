...
function Caption({
  ...
  // Retro rubberhose: chunky Cooper headline (dark brown) with ONE word/phrase
  ...
  // Hand-drawn editorial style: flat-poster slide with sans headline + one
  // brush-script accent phrase. Label is dropped (the corner wordmark fills
  // that role); text is left-aligned with explicit high-contrast colors.
  const styled = !!slide.bgColor;
  if (styled) {
    const headlineColor = slide.onDark ? "#FFFFFF" : NAVY;
    const scriptColor = slide.scriptColor || CORAL;
    const scriptText = pickText(slide.scriptPhrase, locale);
    return (
      <div style={{ textAlign: "left", position: "relative", width: "100%" }}>
        <EditableText
...
export function SlideCanvas({
  ...
  return (
    ...
    >
      <SlideBackground slide={slide} cW={cW} cH={cH} theme={theme} appName={appName} index={0} />
      <SlideElements
        slide={slide}
        device={device}
        orientation={orientation}
    ...
    >
      {slides.map((slide, index) => {
        ...
        return (
          ...
          >
            <SlideBackground slide={slide} cW={cW} cH={cH} theme={theme} appName={appName} index={index} />
            {showGuides && <ScreenGuide cW={cW} cH={cH} index={index} active={active} />}
          </div>
        );
      })}
...
function SlideBackground({
  ...
  // Retro rubberhose: warm flat block + paper grain (multiply) + ink doodles +
  // mascot + chunky lowercase wordmark.
  if (slide.style === "retro") {
    // Mascot is drawn in the element layer (SlideElements) so it sits in front
    // of the phone. Background carries the flat block, grain, doodles, wordmark.
    return (
      <div style={{ position: "absolute", inset: 0, overflow: "hidden", background: slide.bgColor || CREAM }}>
        <RetroGrain />
        <InkDoodles index={index} cW={cW} cH={cH} />
        {appName ? <RetroWordmark name={appName} cW={cW} cH={cH} /> : null}
      </div>
    );
  }

  // Hand-drawn editorial style: flat poster block + grain + corner wordmark +
  // scattered doodles. No theme gradient/blobs.
  if (slide.bgColor) {
    const wordColor = slide.onDark ? "#FFFFFF" : NAVY;
    const doodleColor = slide.doodleColor || (slide.onDark ? CREAM : CORAL);
    return (
      <div style={{ position: "absolute", inset: 0, overflow: "hidden", background: slide.bgColor }}>
        <NoiseOverlay />
        <Doodles index={index} cW={cW} cH={cH} color={doodleColor} />
        {appName ? <Wordmark name={appName} color={wordColor} cW={cW} cH={cH} /> : null}
      </div>
    );
  }
...
function SlideElements({
  ...
  // Retro style swaps the black mockup for the cream inked bezel (same aspect).
  const ActiveFrame =
    slide.style === "retro"
      ? (p: { src: string; alt?: string; style?: React.CSSProperties; hideEmpty?: boolean }) => (
          <RetroPhone {...p} aspect={frameAspect} />
        )
      : Frame;
  const inverted = !!slide.inverted;
  ...
  function renderDevice(id: "device" | "deviceSecondary", rect: Rect, src: string, extraStyle?: React.CSSProperties) {
    const saved = slide.transforms?.[id];
    // Hand-drawn style: tilt comes from slide.tilt unless an explicit transform
    // rotation is set; warm two-stack drop-shadow per slide bg.
    const rotation = saved?.rotation ?? (id === "device" ? slide.tilt ?? 0 : 0);
    const shadowFilter =
      slide.bgColor && id === "device"
        ? `drop-shadow(0 ${cW * 0.028}px ${cW * 0.046}px rgba(${slide.shadowRgba || "8,12,30,0.5"})) drop-shadow(0 ${cW * 0.005}px ${cW * 0.011}px rgba(0,0,0,0.18))`
        : undefined;
    ...
    return (
      ...
      >
        <ActiveFrame
          src={src}
          hideEmpty={hideEmpty}
          style={{ width: "100%", height: "100%", filter: shadowFilter, ...extraStyle }}
        />
      </Movable>
    );
  }
  ...
  return (
    <>
      ...
      {renderCaption()}
      {slide.style === "retro" && slide.mascot ? (
        <Mascot
          cW={cW}
          cH={cH}
          x={(screenX + (slide.mascot.x ?? 0.02) * cW) / cW}
          y={slide.mascot.y ?? 0.6}
          size={slide.mascot.size ?? 0.3}
