...
Journey 主螢幕是「Knowledge Journey」——時間軸 / trip 兩種瀏覽模式 + 搜尋 + 篩選。視覺上以**列表內容為主**，已大量使用 `colorScheme.X`，剩下少量 hardcoded `AppColors.primary` / `AppColors.error` / `Colors.white` 需 token 化。
...
| 議題 | 決定 |
|---|---|
| Journey 主標題（"journey.title"，40/bold）| 改 `Theme.of(context).textTheme.displayLarge`（與 explore 相同）|
