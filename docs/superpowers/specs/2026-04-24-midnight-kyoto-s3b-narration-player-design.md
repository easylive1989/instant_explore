...
- ❌ Save-to-passport **路由目標**（passport_success 螢幕另一個 mini-sprint）
...
| `save_to_passport_button.dart` | 84 | ✅ `AppColors.amber` deprecated | 大 |
...
| Save-to-passport 按鈕 | 改用 `PillButton(secondary, fullWidth, icon: Icons.bookmark_add)`；移除 `AppColors.amber`，icon 色由 `secondary` variant 自動套用 `onSurface` |
...
### `save_to_passport_button` 沒有獨立測試
保持現狀，此次改動視覺優先。
...
| 風險 | 影響 | 緩解 |
|---|---|---|
| `PillButton` icon-in-button 變化讓 save-to-passport 失去「特殊感」 | 中 | 視 UI review 後決定是否回頭加 hero variant；S3-b 不解決 |
...
5. **跑起來看**：play 按鈕電光藍 + shadow、進度條乾淨、transcript active 段有左側電光藍直條 + 加大字、save-to-passport 是 secondary pill button、grounding info 是 ghost icon 圓鈕。
