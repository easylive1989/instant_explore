...
## 2026-07-11 定案：改回本地 HTML（Notion 輸出已移除）

Notion 版實際使用後，使用者決定改回**本地單檔 HTML**（操作更方便）。最終形態：

- `uv run lorescape-dashboard` → 收集 → 產 `dashboard/out/index.html` → 自動開瀏覽器
  （`--no-open` 可關）。collectors 與快取機制不變。
- 版面（依 dataviz skill 規範，light/dark 雙模式、無 JS 依賴）：
  健康燈號 stat tiles、Epic 進度條、待部署 callout、**Backlog 三欄看板**
  （待辦/進行中/已完成；本月完成顯示、更早完成收摺疊，Done 不堆積）、
  部署表（app 列 fallback 讀改名前 `deploy.yml` 的歷史）、測試統計＋失敗清單、
  E2E 案例、產品數據卡（stat tiles＋30 天 sparkline＋近 7 天明細摺疊）、每日故事。
- `notion_writer.py` 已刪除；使用者 Notion 上已建的頁面/資料庫/view 保留由使用者
  處置。下節（Notion 結構）僅留作歷史記錄。

## ~~2026-07-11 改版：子頁面 + database 結構~~（已棄用，見上節）

上線後使用者要求不要全部攤在單頁，改用子頁面與 database，並讓資料可持續累積：

- **主頁**：子頁面導覽 + 健康燈號卡 + 部署狀態表 + 每日故事。
- **📋 Backlog 子頁**：`Features` database（屬性：編號/編號數/Epic/狀態/
  Sprint/本期/進度/狀態備註；row 內文 = tasks to-do）＋ Epic 進度與待部署。
  rows 以編號 upsert，消失的 feature 會 archive。
  **看板**：「本期看板」view（group by 狀態、filter 本期 ✓、依編號數排序）。
  Sprint 規則——未完成永遠掛當前月；已完成鎖在完成月（從狀態文字解析）。
  `本期` checkbox 由工具每次維護（= Sprint 是否當前月），過月後舊 Done 自動
  掉出看板，避免堆積；view 的 filter 是靜態的所以必須用 checkbox 而非月份值。
- **🧪 測試 子頁**：套件統計表 + 失敗清單 + E2E 案例。
- **📈 產品數據 子頁**：每來源一個 database（gsc/ga4/ig/revenuecat/stores/
  narration/retention），日期為 title key **持續 upsert 累積**（補缺日期 +
  更新最近 3 天），這是「metrics 從 Google Sheet 遷 Notion」的第一步；
  子頁內容保留 headline 數字卡（近 7 天表由 database 取代）。
- **寫入策略修訂**：每次重寫的是「內容 blocks」，`child_page` /
  `child_database` 永不刪除；子頁面與 database 首次執行自動建立、之後重用。
- 看板 view 由 Claude 的 Notion 工具一次性建立（API 無法建 view）。
