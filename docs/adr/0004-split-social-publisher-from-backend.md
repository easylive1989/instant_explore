...
- **`scripts/` 依賴改指 `publisher`**：`scripts/pyproject.toml` 的 path
  ...
  `backend/.env` 只保留行銷工具用不到 publisher 套件的設定（如
  `UNSPLASH_ACCESS_KEY`、`METRICS_SHEET_ID`），metrics/unsplash 腳本仍讀它。
...
`BRAND_HANDLE_IG` / `GEMINI_*` 等值搬過去；`backend/.env` 保留
`UNSPLASH_ACCESS_KEY`、`METRICS_SHEET_ID` 等行銷工具設定，metrics/unsplash
腳本仍讀它）：
